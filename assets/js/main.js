...
function contarFrequencia(sorteados) {
    const frequencia = {};
    sorteados.forEach(sorteio => {
...
function numerosMaisFrequentes(sorteados, quantidade = 12) {
    const frequencia = contarFrequencia(sorteados);
    const numerosOrdenados = Object.entries(frequencia).sort((a, b) => b[1] - a[1]);
    return numerosOrdenados.slice(0, quantidade).map(item => parseInt(item[0]));
}
...
function gerarSugestoes(quantidadeSugestoes = 10, numerosPorJogo = 6) {
    ...
    while (sugestoes.length < quantidadeSugestoes) {
        let sugestao = [];

        // Embaralha os números sorteados
        let numerosEmbaralhados = [...numerosSorteados];
        for (let i = numerosEmbaralhados.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [numerosEmbaralhados[i], numerosEmbaralhados[j]] = [numerosEmbaralhados[j], numerosEmbaralhados[i]];
        }

        // Seleciona os primeiros numerosPorJogo do array embaralhado
        sugestao = numerosEmbaralhados.slice(0, numerosPorJogo);

        // Preenche com números aleatórios se necessário
        while (sugestao.length < numerosPorJogo) {
            const numeroAleatorio = Math.floor(Math.random() * 60) + 1;
            if (!sugestao.includes(numeroAleatorio)) {
                sugestao.push(numeroAleatorio);
            }
        }

        sugestao.sort((a, b) => a - b);

        // Verifica se a sugestão já existe na lista de sugestões
        if (!sugestoes.some(s => JSON.stringify(s) === JSON.stringify(sugestao))) {
            sugestoes.push(sugestao);
        }
    }

    document.getElementById('output').innerText = 'Sugestões de jogos:\n' + sugestoes.map((s, i) => `Sugestão ${i + 1}: ${s.join(', ')}`).join('\n');
}

// Função para verificar se um sorteio já existe na lista
function sorteioExiste(novoSorteio) {
    return sorteiosMegaSena.some(sorteio => JSON.stringify(sorteio) === JSON.stringify(novoSorteio));
}

// Função para adicionar números escolhidos pelo usuário
function adicionarNumerosEscolhidos() {
    const sequenciaDigitada = prompt("Digite a sequência de números escolhidos separados por espaço (ou 'q' para cancelar):");
    if (sequenciaDigitada && sequenciaDigitada.toLowerCase() !== 'q') {
        const numeros = sequenciaDigitada.split(' ').map(Number);
        if (numeros.every(num => !isNaN(num))) {
            numerosEscolhidos.push(...numeros);
            alert("Números adicionados com sucesso!");
        } else {
            alert("Entrada inválida. Por favor, digite apenas números separados por espaço.");
        }
    }
}

// Função para exibir números escolhidos pelo usuário
function exibirNumerosEscolhidos() {
    if (numerosEscolhidos.length > 0) {
        alert("Números escolhidos pelo usuário: " + numerosEscolhidos.join(', '));
    } else {
        alert("Ainda não há números escolhidos pelo usuário.");
    }
}

// Função para inserir um novo sorteio
function inserirSorteio() {
    const sequenciaDigitada = prompt("Digite a sequência de números do sorteio separados por espaço (ou 'q' para cancelar):");
    if (sequenciaDigitada && sequenciaDigitada.toLowerCase() !== 'q') {
        const novoSorteio = sequenciaDigitada.split(' ').map(Number);
        if (novoSorteio.every(num => !isNaN(num))) {
            if (sorteioExiste(novoSorteio)) {
                alert("Este sorteio já está na lista de resultados.");
            } else {
                sorteiosMegaSena.push(novoSorteio);
                alert("Sorteio adicionado com sucesso!");
